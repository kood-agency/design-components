...
- accent-foreground — 라이트 `#0A1724`, 다크 `#C8DCEC`
- 포커스 링 — 라이트 `#0A1724`, 다크 `#FFFFFF`

자체 테마를 덮어쓴 소비자는 역할 묶음(캔버스/표면/동작)을 다시 확인하세요. 링크와
선택 강조에 쓰는 `--accent-foreground`가 파랑 `#2258CC`에서 잉크 `#0A1724`로,
포커스 링 `--ring`도 파랑 `#2861DB`에서 잉크로 바뀌었습니다. 이전 파랑 값을 accent
묶음에만 남겨 두면 primary와 accent가 서로 다른 색으로 보입니다.
