...
shadcn 컴포넌트는 원본 그대로 `src/shadcn`에 들어 있습니다.
...
- 공개 `Button`은 Base UI 기반 자체 버튼입니다.
  shadcn의 `shadcn/button`도 함께 들어 있지만 `Calendar` 같은 내부 의존용이라 공개 export에는 포함하지 않았습니다.
- 문서용 조합(`data-table`, `date-picker`, `toast`, `toaster`, `typography`)과
  레지스트리에 없는 항목(`questionnaire`)은 빠져 있습니다.
